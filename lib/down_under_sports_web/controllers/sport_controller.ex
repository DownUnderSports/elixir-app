defmodule DownUnderSportsWeb.SportController do
  ...
  def index(conn, _params) do
    sports = Metas.list_sports()
    render(conn, "index.html", sports: sports)
  end

  def new(conn, _params) do
    changeset = Metas.change_sport(%Sport{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"sport" => sport_params}) do
    case Metas.create_sport(sport_params) do
      {:ok, sport} ->
        conn
  ...
  def update(conn, %{"id" => id, "sport" => sport_params}) do
    ...
    case Metas.update_sport(sport, sport_params) do
      {:ok, sport} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    sport = Metas.get_sport!(id)
    {:ok, _sport} = Metas.delete_sport(sport)
